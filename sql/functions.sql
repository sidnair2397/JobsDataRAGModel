...
CREATE FUNCTION fn_ParseSalaryMin(@salary_range NVARCHAR(255))
RETURNS DECIMAL(18,2)
AS
BEGIN
    DECLARE @Cleaned NVARCHAR(255) = REPLACE(REPLACE(UPPER(@salary_range), '$', ''), 'K', '');
...
CREATE FUNCTION fn_ParseSalaryMax(@salary_range NVARCHAR(255))
RETURNS DECIMAL(18,2)
AS
BEGIN
    DECLARE @Cleaned NVARCHAR(255) = REPLACE(REPLACE(UPPER(@salary_range), '$', ''), 'K', '');
...
CREATE FUNCTION fn_ClassifySentiment(@sentiment_score DECIMAL(5,4))
RETURNS NVARCHAR(10)
AS
BEGIN
...
CREATE FUNCTION fn_CleanBenefits(@benefits NVARCHAR(MAX))
...
CREATE FUNCTION fn_GetSkillCount(@job_id BIGINT)
RETURNS INT
AS
BEGIN
